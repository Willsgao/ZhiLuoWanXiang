...
function createNetwork(data) {
    const container = document.getElementById('network');
        ...
        // 调试：打印节点和边的数量
        console.log('[createNetwork] nodes count:', data.nodes ? data.nodes.length : 0);
        console.log('[createNetwork] edges count:', data.edges ? data.edges.length : 0);
        if (data.edges && data.edges.length > 0) {
            console.log('[createNetwork] first edge sample:', JSON.stringify(data.edges[0]));
            console.log('[createNetwork] all edges:', JSON.stringify(data.edges));
        } else {
            console.warn('[createNetwork] ⚠️ edges 数组为空或不存在! data.edges =', data.edges);
        }
